...
export async function fetchAavePools(address: string) {
  ...
  try {
    ...
    for await (const chain of appSettings.chains) {
      ...
      if (poolConfig["aaveV3"][chain.id]) {
        //Contract address
        const uiPoolDataProviderAddress =
          poolConfig["aaveV3"][chain.id].uiPoolDataProviderAddress;
        const lendingPoolAddressProvider =
          poolConfig["aaveV3"][chain.id].lendingPoolAddressProvider;

        // View contract used to fetch all reserves data (including market base currency data), and user reserves
        // Use UiPoolDataProvider for AaveV3
        ...
        const reserves = await queryClientServer.fetchQuery({
          queryKey: ["reserves", "aaveV3", chain.id],
          queryFn: () =>
        ...
        result["aaveV3"][chain.id] = balance;
      }
      console.groupEnd();
    }
    console.groupEnd();

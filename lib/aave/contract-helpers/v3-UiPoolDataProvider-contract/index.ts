...
import { abi as UiPoolDataProvider_ABI } from "./abi";
...
export interface UiPoolDataProviderInterface {
  getReservesList: (args: ReservesHelperInput) => Promise<readonly string[]>;
  getReservesData: (args: ReservesHelperInput) => Promise<ReservesData>;
  getUserReservesData: (
    args: UserReservesHelperInput
  ) => Promise<UserReserveData>;
  getEModes: (args: ReservesHelperInput) => Promise<readonly EModeData[]>;
  getEModesHumanized: (
...
export class UiPoolDataProvider implements UiPoolDataProviderInterface {
  ...
  private readonly wagmiConfigChainIndex: number;
  private readonly uiPoolDataProviderAddress: Address;
  ...
  public constructor(context: UiPoolDataProviderContext) {
    ...
    this.wagmiConfigChainIndex = config.chains.findIndex(
      (chain) => chain.id === context.chainId
    );
  }
  ...
  }: ReservesHelperInput): Promise<readonly string[]> {
    if (!isAddress(lendingPoolAddressProvider)) {
      throw new Error("Lending pool address is not valid");
    }

    return readContract(config, {
      abi: UiPoolDataProvider_ABI,
      address: this.uiPoolDataProviderAddress,
      functionName: "getReservesList",
      args: [lendingPoolAddressProvider],
      chainId: config.chains[this.wagmiConfigChainIndex].id,
    });
  }
  ...
  public async getUserReservesData({
    ...
    return readContract(config, {
      ...
      args: [lendingPoolAddressProvider, user],
      chainId: config.chains[this.wagmiConfigChainIndex].id,
    });
  }
  ...
  public async getEModes({
    lendingPoolAddressProvider,
  }: ReservesHelperInput): Promise<readonly EModeData[]> {
    if (!isAddress(lendingPoolAddressProvider)) {
      throw new Error("Lending pool address is not valid");
    }
